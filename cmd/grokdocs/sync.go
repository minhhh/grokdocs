...
func runSyncPreRun() error {
	if syncAll && syncCollection != "" {
		return fmt.Errorf("--all and --collection flags are mutually exclusive")
	}
	return nil
}

func runSync(startDir string) error {
	if startDir == "" {
...
var syncCmd = &cobra.Command{
	...
	PreRun: func(cmd *cobra.Command, args []string) {
		if err := runSyncPreRun(); err != nil {
			util.Logger.Error().Err(err).Msg("invalid flags")
			os.Exit(1)
		}
	},
	Run: func(cmd *cobra.Command, args []string) {
		if err := runSync(projectPath); err != nil {
			os.Exit(1)
		}
	},
}
